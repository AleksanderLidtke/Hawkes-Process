...
baseEventTimes = all_counts.index.astype(np.int64)/ 10**9 # Times when the events take place as seconds from epoch.
baseEventTimes = baseEventTimes.astype(np.float64) # Convert to floats to avoid errors in division etc.
...
for i in range(len(empirical_1min.values)): # Deal with many events occurring at the same time - need to distinguish between them by splitting each batch of events into distinct events taking place at almost the same time.
    if not np.isnan(empirical_1min.values[i]):
        for j in range(empirical_1min.values[i]):
            eventTimes.append( empirical_1min.index.astype(np.int64)[i] + 0.000001*(j+1)) # For every event that occurrs at this epoch enter a dummy event very close to it in time that will increase the conditional intensity.
...
#TODO: scale these times to true epochs.
...
integralsOfConditionalIntensity = np.zeros( baseEventTimes.size )
for i in range( baseEventTimes.size ): # Find integral of the conditional intensity at eery time, index i.
...
interArrivalTimes = np.zeros( baseEventTimes.size ) # Times between consecutive events.
interArrivalTimes[1:] = baseEventTimes[1:] - baseEventTimes[:-1] # First event is when we start counting time, no interarrival time for it.
...
ax.plot(empirical_1min.index.to_pydatetime(), empirical_1min.values, color='blue', linestyle='solid', marker=None, markerfacecolor='blue', markersize=12)
...
ax.plot(timesOfInterest, conditionalIntensitiesPlotting, color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
fittedPlot = matplotlib.lines.Line2D([],[],color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
...
""" MAKE A QUANTILE PLOT OF THE INTERARRIVAL TIMES. """
" Process the data and compute the quantiles. "
orderStatistics = integralsOfConditionalIntensity;
orderStatistics2 = interArrivalTimes;
...
smapleQuantiles=np.zeros( orderStatistics.size ) # Quantiles of the integralsOfConditionalIntensity i.e. the rescaled time.
smapleQuantiles2=np.zeros( orderStatistics2.size ) # Quantiles of the interArrivalTimes.
...
" Make a quantile plot of inter-arrival times. "
fig3 = matplotlib.pyplot.figure()
ax3 = fig3.gca(aspect="equal")

fig3.suptitle(r"$Quantile\ plot\ of\ inter-arrival\ times$", fontsize=20)
ax3.grid(True)
ax3.set_xlabel(r'$Sample\ fraction\ (\%)$',fontsize=labelsFontSize)
ax3.set_ylabel(r'$Ordered\ values$',fontsize=labelsFontSize)
matplotlib.rc('xtick', labelsize=ticksFontSize) 
matplotlib.rc('ytick', labelsize=ticksFontSize)

distScatter = ax3.scatter(smapleQuantiles2, orderStatistics2, c='blue', marker='o') # If these are close to the straight line with slope line these points come from a normal distribution.

ax3.plot(smapleQuantiles2, smapleQuantiles2, color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
normalDistPlot = matplotlib.lines.Line2D([],[],color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
     
fig3.legend([normalDistPlot, distScatter], [r'$Normal\ distribution$', r'$Inter-arrival\ times$'])
fig3.show()

" Make a quantile plot of rescaled times. "
fig4 = matplotlib.pyplot.figure()
ax4 = fig4.gca(aspect="equal")

fig4.suptitle(r"$Quantile\ plot\ of\ rescaled\ times$", fontsize=20)
ax4.grid(True)
ax4.set_xlabel(r'$Sample\ fraction\ (\%)$',fontsize=labelsFontSize)
ax4.set_ylabel(r'$Ordered\ values$',fontsize=labelsFontSize)
matplotlib.rc('xtick', labelsize=ticksFontSize) 
matplotlib.rc('ytick', labelsize=ticksFontSize)

distScatter = ax4.scatter(smapleQuantiles, orderStatistics, c='blue', marker='o') # If these are close to the straight line with slope line these points come from a normal distribution.

ax4.plot(smapleQuantiles, smapleQuantiles, color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
normalDistPlot = matplotlib.lines.Line2D([],[],color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
     
fig4.legend([normalDistPlot, distScatter], [r'$Normal\ distribution$', r'$Rescaled\ times$'])
matplotlib.pyplot.show()

" QQ plot of inter-arrival times and the rescaled times. "
fig5 = matplotlib.pyplot.figure()
ax5 = fig5.gca(aspect="equal")

fig5.suptitle(r"$Quantile\ -\ Quantile\ plot$", fontsize=20)
ax5.grid(True)
ax5.set_xlabel(r'$Quantiles\ of\ the\ rescaled\ time$',fontsize=labelsFontSize)
ax5.set_ylabel(r'$Quantiles\ of\ the\ inter-arrival\ times$',fontsize=labelsFontSize)
matplotlib.rc('xtick', labelsize=ticksFontSize) 
matplotlib.rc('ytick', labelsize=ticksFontSize)

distributionScatter = ax5.scatter(smapleQuantiles, smapleQuantiles2, c='blue', marker='x') # If these are close to the straight line with slope line these points come from a normal distribution.

ax5.plot(smapleQuantiles, smapleQuantiles, color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
normalDistPlot2 = matplotlib.lines.Line2D([],[],color='red', linestyle='solid', marker=None, markerfacecolor='red', markersize=12)
     
fig5.legend([normalDistPlot2, distributionScatter], [r'$Normal\ distribution$', r'$Comparison\ of\ datasets$'])
fig5.show()
